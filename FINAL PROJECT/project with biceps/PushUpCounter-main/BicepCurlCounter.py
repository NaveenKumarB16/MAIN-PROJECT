...
while cap.isOpened():
    ...
    if len(lmList) != 0:
        ...
        wrist = detector.findAngle(img, 14, 16, 20)
        
        #Percentage of success of Bicep Curl
        per = np.interp(elbow, (40, 160), (0, 100))
        
        #Bar to show Bicep Curl progress
        bar = np.interp(elbow, (40, 160), (380, 50))

        #Check to ensure right form before starting the program
        if elbow > 160 and shoulder < 5:
            form = 1
    
        #Check for full range of motion for the pushup
        if form == 1:
            if per == 0:
                if elbow <= 40 and shoulder < 5:
                    feedback = "Down"
                    if direction == 0:
                        count += 0.5
                        direction = 1
